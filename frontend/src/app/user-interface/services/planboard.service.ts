...
export class PlanboardService {
  ...
  assignItemToPriceEntry(listUuid: string, item: ShoppingListItem, price: Price | undefined, tripId?: number | undefined): void {
    if (price) {
      ...
      this.itemAssignments.update(c => ({
        ...
        [listUuid]: {
          ...
          [item.productUuid ?? item.productGroupUuid]: {
            tripId: tripId ?? Math.max(0, ...Object.values(c).flatMap(Object.values).map(v => v?.tripId ?? -1)) + 1,
            price: price,
          },
        }
      }));
